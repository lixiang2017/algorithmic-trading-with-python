...
from src.pypm.portfolio import PortfolioHistory, Position, Symbol, Dollars
...
class SimpleSimulator(object):
    ...
    def make_all_valid_lookup(_idx: Callable):
        ...
        return lambda row, symbol: (
            not pd.isna(row[_idx(symbol, "pref")])
            and not pd.isna(row[_idx(symbol, "signal")])
            and not pd.isna(row[_idx(symbol, "price")])
        )
    ...
    def _assert_equal_columns(*args: Iterable[pd.DataFrame]):
        ...
        for arg in args[1:]:
            assert (
                set(arg.columns.values) == column_names
            ), "Found unequal column names in input data frames."

    def simulate(
        self, price: pd.DataFrame, signal: pd.DataFrame, preference: pd.DataFrame
    ):
        """
        Runs the simulation.
        ...
        for row in df.itertuples():
            ...
            to_buy = [
                s
                for s in symbols
                if row[_idx(s, "signal")] == 1 and not s in active_positions_by_symbol
            ]
